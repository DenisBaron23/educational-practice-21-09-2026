...
def save(self):
        ...
        if self.on_save is None:
            messagebox.showinfo(
                ...
                "Сохранение в базу не подключено.",
                parent=self,
            )
            return

        if not self.on_save(self.partner_id, data):
            return

        self.saved = True

        if self.partner_id is None:
            title = "Добавление выполнено"
            text = "Новый партнер успешно добавлен в базу данных."
        else:
            title = "Изменения сохранены"
            text = "Данные партнера успешно обновлены."
   ...
   def cancel(self, event=None):
        has_changes = self.snapshot() != self.initial_state

        if has_changes:
            should_close = messagebox.askyesno(
                "Несохраненные изменения",
                "Вы изменили данные партнера.\n\n"
                "Если закрыть карточку, несохраненные изменения "
                "будут потеряны без возможности восстановления.\n\n"
